...
interface AddSeasonalFormValues {
    name: string;
    price: string;
}

export default function AddSeasonalDialog({
    isOpen,
    onClose,
}: {
    isOpen: boolean;
    ...
    const initialValues: AddSeasonalFormValues = {
        name: '',
        price: '',
    };

    const handleSubmit = (values: AddSeasonalFormValues) => {
            ...
            <Dialog onClose={onClose} className="fixed inset-0 z-50 flex items-center justify-center">
                ...
                >
                   <Dialog.Panel className="bg-white w-full max-h-[90vh] overflow-y-auto sm:h-auto max-w-3xl sm:max-w-4xl md:max-w-5xl lg:max-w-6xl rounded-none sm:rounded-lg p-4 sm:p-6 shadow-lg z-10">
                        ...
                        <Dialog.Title className="text-md font-bold mb-6">Add seasonal pricing</Dialog.Title>
