...
import { useGetOrdersHistoryQuery, useCancelOrderMutation } from "@src/Hooks";
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-2 items-center">
        {OrdersTabs.map(({ tabName, filter: _filter }, i) => (
          <>
            <div onClick={() => setFilter(_filter)} className="px-4 py-2">
              <TranslationText
                className={`${
                  filter === _filter
      ...
      {(() => {
        const { data: orders } = useGetOrdersHistoryQuery(filter || "all");

        const { mutate: CancelOrder, isLoading: orderCancelationLoading } =
                  ...
                  orders.map((props, i) => {
                    ...
                    return (
                      <Tr className="cursor-pointer" key={i}>
                        ...
                        <Td>{orderName}</Td>
                        <Td>
                          {new Date(orderDeliveryDate).toLocaleDateString(
                            "en",
                            {
                              day: "2-digit",
                              month: "2-digit",
