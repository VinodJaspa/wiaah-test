...
it("render as expeted with all parameters", async () => {
    const wrapper = shallow(
        <ProductFilter 
            ...
            shipping={[{label:'Click and Collect', value:'click_and_collect'},{label:"New Method", value:"new_method"}]}
            brands={[{label:"First Brand", value: "first_brand"},{label:"Seconde Brand", value: "seconde_brand"}]}
        />
    );
...
describe("country an city selector works",() =>{
    ...
    it("country an city selector works", () => {
        ...
        expect(JSON.stringify(wrapper.find('#cityselect').prop('options'))).toContain('Select country first!');
